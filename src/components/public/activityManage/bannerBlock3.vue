...
<script>
    export default {
        ...
        created() {
            this.imgSrc = this.bannerInfo[2].image
            this.type = this.bannerInfo[2].item_type
            this.dataValue = this.bannerInfo[2].data
        },
        // computed:{
        //     banner() {          // banner信息
        ...
        methods: {
            ...
            showSubmit () {
                this.$axios.post('Activity/update_banner', {
                    id: this.bannerId || this.bannerInfo[2].id,
                    item_type: this.type,
                    data:this.dataValue
                })
                    .then(res => {
            ...
            sureDelDialog() {                
                this.$axios.post('Activity/del_banner',{
                    id:this.bannerInfo[2].id
                })
                    .then(res=>{ 
                        if (res.data.code == 200) {
                            this.$message.success(res.data.message)
                            this.dialogVisible = false
                            this.$emit('listenDelBanner3',2)
                            location.reload()
                ...
                this.$upload({
                    ...
                    .then(res => {
                        if (res.data.code == 200) {
                            ...
                            this.banner3Info = res.data.data
                            this.$emit('listenBanner3',this.banner3Info)
                        }
                    })
                    .catch(err => {
                        console.log(err)
