...
<script>
    export default {
        ...
        created() {
            this.imgSrc = this.bannerInfo[0].image
            this.type = this.bannerInfo[0].item_type
            this.dataValue = this.bannerInfo[0].data
        },
        computed:{
            banner() {          // banner信息
        ...
        methods: {
            ...
            showSubmit () {
                this.$axios.post('Activity/update_banner', {
                    id: this.bannerId || this.bannerInfo[0].id,
                    item_type: this.type,
                    data:this.dataValue
                })
                    .then(res => {
            ...
            sureDelDialog() {                
                this.$axios.post('Activity/del_banner',{
                    id:this.bannerInfo[0].id
                })
                    .then(res=>{ 
                        if (res.data.code == 200) {
                            this.$message.success(res.data.message)
                            this.dialogVisible = false
                            this.$emit('listenDelBanner1',0)
                            location.reload()
                        } else {
            ...
            changeUpload(e) {
                ...
                this.$upload({
                    ...
                    .then(res => {
                        if (res.data.code == 200) {
                            this.banner1Info = res.data.data
                            this.$emit('listenBanner1',this.banner1Info)
                        }
                    })
                    .catch(err => {
                        console.log(err)
