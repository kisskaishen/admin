<template>
    <div>
        <el-breadcrumb separator=">">
            <el-breadcrumb-item to="/agentManage">代理商</el-breadcrumb-item>
        ...
        <div class="contentDiv">
            <el-tabs v-model="activeName" type="border-card" @tab-click="switchChange">
                <el-tab-pane label="提现记录" name="first">
                    <div class="moneyType">
                        <el-row>
                            <el-col v-for="item in group" :key="item.title" >
                                <div @click="clickType(item)">
                                    <i class="fl"></i>
                                    <div class="fr">
                                        <p>{{item.title}}</p>
                    ...
                    <table>
                        ...
                        <tr v-for="item in list">
                            ...
                            <td>{{item.time}}</td>
                            <td>{{item.status}}</td>
                            <td><span @click="isSee(item)" style="color: #e4393c;">{{item.issee}}</span></td>
                        </tr>
                    </table>
...
<script>
    export default {
        ...
        methods: {
            ...
            isSee(val) {
                if (val.issee == '查看') {
                    this.$router.push({path:'/agentManage/watchCash',query:{id:val.id,member_id:val.member_id}})
                } else if (val.issee == '取消') {
                    this.$axios.post('Agent/refuse_cash', {
                        id: val.id,
                        type: val.type_num,
                        member_id: this.memberId,
                        member_type: this.memberType,
